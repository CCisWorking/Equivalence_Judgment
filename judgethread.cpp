...
//            std::cerr<<"files_path[i][j]:"<<files_path[i][j]<<std::endl;
    ...
    for(int i=0;i<folders_cnt;i++){
        for(int j=0;j<folders_files_cnt[i];j++){
            for(int k=j+1;k<folders_files_cnt[i];k++){
                ...
                if(re) eq<<files_path[i][j]<<","<<files_path[i][k]<<std::endl;
                else neq<<files_path[i][j]<<","<<files_path[i][k]<<std::endl;
            }
        }
    }
    eq.close();
    neq.close();
//    for(int i=0;i<folders_cnt;i++){
//        for(int j=0;j<paras_cnt[i];j++){
//            std::cerr<<"type:"<<paras[i][j]<<std::endl;
//            if(paras[i][j]!=0){
//                std::cerr<<"left:"<<paras_left[i][j]<<std::endl;
//                std::cerr<<"right:"<<paras_right[i][j]<<std::endl;
//            }
//        }
//        std::cerr<<std::endl;
//    }
...
//    std::cerr<<order2<<std::endl;
    ...
    system(_5);
    system(_6);
    std::string order7="diff "+folderpath+folders[folder]+"/"+"output1.txt"+" "+folderpath+folders[folder]+"/"+"output2.txt > "+folderpath+folders[folder]+"/"+"out.txt";
